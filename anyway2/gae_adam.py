...
transition = np.dtype([('s', np.float64, (4, 28, 28)), ('a', np.float64, (4,)), ('r', np.float64),
                       ('s_', np.float64, (4, 28, 28)), ('a_logp', np.float64), ('v', np.float64)])
...
class sumomodel():
    ...
    def __init__(self):
        ...
        self.gamma = 0.99
        self.lam = 0.95
        self.path_t7 = './checkpoint/p_gae.t7'
        self.path_wt = './csvfiles/averange_gae.csv'
        self.path_lsa = './csvfiles/loss_a_gae.csv'
        self.path_lsv = './csvfiles/loss_v_gae.csv'
        self.path_ac = './csvfiles/ac_gae.csv'
    ...
    def getgae(self, f_v):
        r = self.memory.buffer['r']
        v = self.memory.buffer['v']

        tds = r - v + np.append(v[1:] * self.gamma, f_v)
        advantages = self.discount(tds, self.gamma * self.lam)
        target_v = advantages + v
        return advantages, target_v
    ...
    def trainmodel(self):
        ...
        adv = torch.tensor(np.array(adv), dtype=torch.double).to(device).view(-1, 1)
        target_v = torch.tensor(target_v, dtype=torch.double).to(device).view(-1, 1)
    ...
    def run(self):
        ...
        while traci.simulation.getMinExpectedNumber() > 0:
            ...
            if (self.step % 2000 == 15):
                ...
                self.out_record = [pict, action, a_logp, self.net(input_d)[1].item()]
