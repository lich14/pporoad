...
class PPOnet(nn.Module):
    ...
    def __init__(self):
        ...
        self.v = nn.Sequential(nn.Linear(128, 2000), nn.ReLU(), nn.Linear(2000, 2000), nn.ReLU(), nn.Linear(2000, 1))
        self.fc = nn.Sequential(nn.Linear(128, 2000), nn.ReLU(), nn.Linear(2000, 2000), nn.ReLU())
...
class sumomodel():
    ...
    def __init__(self):
        ...
        self.memory = store()
        self.gamma = 0.95
        self.path_t7 = './checkpoint/p_lstm_begin' + str(args.bound) + '.t7'
        self.path_wt = './csvfiles/averange_lstm_begin' + str(args.bound) + '.csv'
        self.path_lsa = './csvfiles/loss_a_lstm_begin' + str(args.bound) + '.csv'
        self.path_lsv = './csvfiles/loss_v_lstm_begin' + str(args.bound) + '.csv'
        self.path_ac = './csvfiles/ac_lstm_begin' + str(args.bound) + '.csv'
    ...
    def for_lstm(self, input):
        input = input.to("cpu").numpy()
        layer = int(np.size(input) / 3136)
        h = torch.tensor(np.zeros([layer, 128]), dtype=torch.double).to(device)
        c = torch.tensor(np.zeros([layer, 128]), dtype=torch.double).to(device)
    ...
    def trainmodel(self):
        ...
        for _ in range(self.PPOepoch):
            for index in BatchSampler(
                ...
                value_loss = F.smooth_l1_loss(self.for_lstm(s[index])[1], target_v[index])
    ...
    def run(self):
        ...
        while traci.simulation.getMinExpectedNumber() > 0:
            ...
            if (self.step % 2000 == 15):
                ...
                with torch.no_grad():
                    input = torch.tensor(pict, dtype=torch.double).to(device)
                    (alpha, beta), _ = self.for_lstm(input)
